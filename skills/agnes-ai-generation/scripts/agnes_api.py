...
def query_video_by_video_id(video_id: str) -> dict[str, Any]:
    """Query video result using video_id (recommended approach).

    Endpoint: GET /agnesapi?video_id=<VIDEO_ID>
    """
    return request_json("GET", f"/agnesapi?video_id={video_id}")
...
def poll_video(video_id: str, timeout: int, interval: int, use_task_id: bool = False) -> dict[str, Any]:
    deadline = time.time() + timeout
    last: dict[str, Any] = {}
    while time.time() < deadline:
        if use_task_id:
            last = request_json("GET", f"/v1/videos/{video_id}")
        else:
            last = query_video_by_video_id(video_id)
        if last.get("error"):
            raise SystemExit(f"Video task {video_id} returned error: {json.dumps(last, ensure_ascii=False)}")
        ...
        if status in {"completed", "failed"}:
            return last
        time.sleep(interval)
    raise SystemExit(f"Timed out waiting for video task {video_id}. Last response: {json.dumps(last)}")


def cmd_video(args: argparse.Namespace) -> None:
    created = request_json("POST", "/v1/videos", video_payload(args))
    video_id = created.get("video_id") or created.get("id")
    use_task_id = args.use_task_id or not created.get("video_id")

    if not args.poll:
        next_steps = []
        if video_id:
            if use_task_id:
                next_steps.append(f"python scripts/agnes_api.py video-get {video_id}")
                next_steps.append(f"python scripts/agnes_api.py video-get {video_id}  # repeat until status is completed")
            else:
                next_steps.append(f"python scripts/agnes_api.py video-get --video-id {video_id}")
                next_steps.append(f"python scripts/agnes_api.py video-get --video-id {video_id}  # repeat until status is completed")
        output_result(
            "video-task",
            created,
        ...
        return
    if not video_id:
        raise SystemExit(f"Video create response did not include video_id or id: {json.dumps(created)}")
    data = poll_video(str(video_id), args.timeout, args.interval, use_task_id)
...
def cmd_video_get(args: argparse.Namespace) -> None:
    if args.video_id:
        data = query_video_by_video_id(args.video_id)
        vid = args.video_id
    else:
    ...
    output_result(
        ...
        urls=urls,
        status=str(data.get("status", "")) if data.get("status") is not None else None,
        next_steps=[] if urls else [f"python scripts/agnes_api.py video-get --video-id {vid}"],
        raw_only=args.raw,
    )
    if data.get("error"):
        raise SystemExit(1)
...
def create_video_case(name: str, payload: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    video_id = created.get("video_id") or created["id"]
    use_task_id = args.video_use_task_id or not created.get("video_id")
    retrieved = (
        poll_video(str(video_id), args.video_timeout, args.video_interval, use_task_id)
        if args.poll_video
        else query_video_by_video_id(str(video_id)) if not use_task_id
        else request_json("GET", f"/v1/videos/{video_id}")
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    video.add_argument("--use-task-id", action="store_true", help="Use legacy task_id endpoint for polling.")
    ...
    video_get.add_argument("task_id", nargs="?", help="Task ID (legacy). Omit if using --video-id.")
    video_get.add_argument("--video-id", help="Video ID (recommended). Overrides positional task_id.")
    ...
    smoke.add_argument("--video-use-task-id", action="store_true", help="Use legacy task_id endpoint for video polling.")
