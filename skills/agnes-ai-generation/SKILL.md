...
Retrieve a video task by video_id (recommended):

```bash
python scripts/agnes_api.py video-get --video-id video_xxxxxx
```

Retrieve a video task by task_id (legacy):
...
- For videos, remember the API is asynchronous: create a task first, then poll or retrieve by video_id (use `--video-id` flag). The create response includes both `video_id` and `task_id`; prefer `video_id` with the new query endpoint `GET /agnesapi?video_id=<VIDEO_ID>`.
